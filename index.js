	...
	messages: [
		{
            ...
            5.  **Beschränkungen:**
                ...
                * Vermeide Slang, umgangssprachliche Ausdrücke, Emojis oder Jugendsprache. response in JSON like this: {response: string; character: 'gaius' | 'octavius'}.
            `,
		},
	],
};
...
document.addEventListener('DOMContentLoaded', () => {
	...
	formElement.addEventListener('submit', async (event) => {
		...
		const regieanweisung = Math.random() < 0.5 ? 'Du bist jetzt Octavius' : 'Du bist jetzt Gaius'; // Zufällige Regieanweisung für den Charakterwechsel
		// @ts-ignore

		messageHistory.messages.push({ role: 'user', content: content });
		console.log("Aktualisierte messageHistory:", messageHistory);
		...
		// createa a copy of the messageHistory to send to the server
		const messageHistoryCopy = JSON.parse(JSON.stringify(messageHistory)); // Deep copy to avoid mutation
		// Füge die Regieanweisung hinzu, um den Charakter zu wechseln
...
function addToChatHistoryElement(mhistory) {
	const htmlStrings = mhistory.messages.map((message) => {
		...
		let additionalClass = message.additionalClass || ''; // Zusätzliche Klasse, falls vorhanden
		return `<div class="message ${senderClass} ${additionalClass}">${message.content}</div>`;
	});
